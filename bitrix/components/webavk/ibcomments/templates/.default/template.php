...
<div class="webavk_ibcomments">
	...
	if ($arParams['USE_FEATURE_VOTE_CONV'] || $arParams['USE_FEATURE_VOTE_FUNC'] || $arParams['USE_FEATURE_VOTE_DESIGN'] || $arParams['USE_FEATURE_VOTE_WORK'] || $arParams['USE_FEATURE_VOTE_PRICE'])
		$isStars = true;
	?>
	<h2><?= GetMessage("WEBAVK_COMP_HEADER") ?> (<?= intval($arResult['NAV_RESULT']->NavRecordCount) ?>)</h2>
	<?
	if ($arResult['ALLOW_ADD']) {
		?>
		<div class="webavk_ibcomments_add_area">
			<a href="#commentform" class="webavk_ibcomments_add_showlink" onclick="return doToggleShowCommentForm(this)"><span><?= GetMessage("WEBAVK_COMP_ADD_COMMENT") ?></span></a>
			<div class="webavk_ibcomments_add_form" id="webavk_ibcomments_add_form" style="display: <? if (count($arResult['POST_COMMENT']) > 0) echo "block"; else echo "none"; ?>;">
						...
						<table>
							...
							if ($arParams['USE_FEATURE_ADVANTAGES']) {
								...
								<tr>
									...
									<td><textarea name="COMMENT[ADVANTAGES]" rows="5" cols="60"><?= htmlspecialchars($arResult['POST_COMMENT']['ADVANTAGES']) ?></textarea></td>
								</tr>
								<?
							}
							...
							if ($arParams['USE_FEATURE_REVIEW']) {
								...
								<tr>
									...
									<td><textarea name="COMMENT[REVIEW]" rows="5" cols="60"><?= htmlspecialchars($arResult['POST_COMMENT']['REVIEW']) ?></textarea></td>
								</tr>
								<?
							}
							...
							if ($arResult['IS_GUEST']) {
								...
								<tr>
									...
									<td>
										<input type="text" name="COMMENT[GUEST_NAME]" value="<?= htmlspecialchars($arResult['POST_COMMENT']['GUEST_NAME']) ?>" size="50"/>
									</td>
								</tr>
								<tr>
									<th><?= GetMessage("WEBAVK_COMP_FORM_FIELD_GUEST_EMAIL") ?>:<span class="required">*</span></th>
									<td>
										<input type="text" name="COMMENT[GUEST_EMAIL]" value="<?= htmlspecialchars($arResult['POST_COMMENT']['GUEST_EMAIL']) ?>" size="50"/>
									</td>
								</tr>
								<?
