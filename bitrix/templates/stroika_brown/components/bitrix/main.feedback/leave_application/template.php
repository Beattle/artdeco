...
<form method="post" action="<?= POST_FORM_ACTION_URI ?>" id="leave_application_form">
    ...
    <label>
        ...
        <input placeholder="<? /*=GetMessage("NAME_DEMO");*/ ?>" type="text" name="name" required="required"
               class="form-input"/>
    </label>
    <label>
...
<script>
    $("#leave_application_form input[type='submit']").click(function (e) {
        ...
        var message = $.trim($("#leave_application_form textarea[name='message']").val());
        ...
        if (email == '') {
            $("#leave_application_form input[name='email']").css({'border': '1px solid #ff0000'});
            dopusk = true;
        } else {
            $("#leave_application_form input[name='email']").css({'border': '1px solid #cccccc'});
            if (!validate(email)) {
                $("#leave_application_form input[name='email']").css({'border': '1px solid #ff0000'});
                mailTo = true;
            }
    ...
    $("#leave_application_form input[name='email']").focus(function (e) {
        $("#leave_application_form input[name='email']").css({'border': '1px solid #cccccc'});
    })
    $("#leave_application_form input[name='email']").focusout(function (e) {
        if ($.trim($("#leave_application_form input[name='email']").val()) == '') {
            $("#leave_application_form input[name='email']").css({'border': '1px solid #ff0000'});
        } else {
            $("#leave_application_form input[name='email']").css({'border': '1px solid #cccccc'});
        }
